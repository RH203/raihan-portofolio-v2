    ...
    return (
        <>
            ...
            <main className="mx-auto max-w-6xl px-6 py-14">
                ...
                {featured && (
                    <Link href={`/blog/${featured.slug}`} className="group mb-16 grid overflow-hidden rounded-3xl border border-surface-200 bg-white md:grid-cols-2">
                        <div className="aspect-[16/10] bg-surface-100 md:aspect-auto">
                            {featured.cover_image && <img src={`/storage/${featured.cover_image}`} alt={featured.title} className="h-full w-full object-cover" />}
                        </div>
                ...
                <section className="grid gap-x-8 gap-y-12 md:grid-cols-2 lg:grid-cols-3" aria-label="Latest stories">
                    {posts.data.map((post) => (
                        <article key={post.id} className="group">
                            <Link href={`/blog/${post.slug}`}>
                                <div className="aspect-[16/10] overflow-hidden rounded-2xl bg-surface-100">
                                    {post.cover_image && <img src={`/storage/${post.cover_image}`} alt={post.title} loading="lazy" className="h-full w-full object-cover transition duration-300 group-hover:scale-[1.03]" />}
                                </div>
                                <div className="mt-5 flex flex-wrap gap-2">{post.tags?.map((tag) => <span key={tag} className="rounded-full bg-surface-100 px-3 py-1 text-xs text-surface-600">{tag}</span>)}</div>
                                <h2 className="mt-3 text-2xl font-bold tracking-tight text-surface-950 group-hover:text-primary-700">{post.title}</h2>
                                ...
                                <div className="mt-4 flex items-center justify-between text-sm text-surface-500">
                                    <span>{post.reading_time} min read</span><ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" />
                                </div>
                            </Link>
                        </article>
                    ))}
