...
export default function AdminLayout({ children, title, description, actions }: AdminLayoutProps) {
    ...
    return (
        <div className="min-h-screen bg-surface-50">
            ...
            <aside
                className={cn(
                    'fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-surface-100 transition-transform duration-300 lg:translate-x-0',
                ...
                <div className="flex h-full flex-col">
                    ...
                    <div className="flex items-center justify-between px-6 py-5 border-b border-surface-100">
                        <Link href="/admin" className="text-lg font-bold text-surface-900">
                            RH<span className="text-primary-600">.</span> <span className="text-sm font-normal text-surface-400">Admin</span>
                        </Link>
                        <button onClick={() => setSidebarOpen(false)} className="lg:hidden p-1 rounded text-surface-400 hover:text-surface-600" aria-label="Close sidebar">
                            <X className="h-5 w-5" />
                        </button>
                    </div>
                    ...
                    <div className="border-t border-surface-100 px-3 py-4 space-y-1">
                        <Link
                            href="/"
                            className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-surface-600 hover:bg-surface-50 transition-colors"
                            target="_blank"
                        >
                        ...
                        <button
                            ...
                            className="flex w-full items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-surface-600 hover:bg-danger-50 hover:text-danger-600 transition-colors"
                        >
